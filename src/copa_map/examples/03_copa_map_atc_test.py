"""This script shows how predictions can be made with the model and plots the resulting data."""
...
if not exists(opt_params_file):
    print("No file with optimized parameters found, unzipping pre trained file...")
    with zipfile.ZipFile(join(data_folder, 'grid_atc_opt_params.zip'), 'r') as zip_ref:
        zip_ref.extractall(data_folder)

assert exists(opt_params_file), "No saved optimized params found. Run 02_copa_map_atc_train.py before this script."
...
def update_scatter(pl, X, c):
    """Update the scatter plots with data from other timestamps"""
    pl.set_offsets(X)
    pl.set_array(c.ravel())


def get_time_str(i):
    """Convert the bin num to a day/hour"""
    ti = t_bins[i]
